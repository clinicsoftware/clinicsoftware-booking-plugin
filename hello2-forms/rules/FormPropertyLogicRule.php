...
class FormPropertyLogicRule implements Rule, DataAwareRule {
	...
	private function checkBaseCondition( $condition ) {
		...
		if ( ! isset( $condition['value'] ) ) {
			...
			$this->conditionErrors[]  = esc_html__( 'missing condition body', 'hello2-forms' );
		...
		if ( ! isset( $condition['value']['property_meta'] ) ) {
			$this->isConditionCorrect = false;
			$this->conditionErrors[]  = esc_html__( 'missing condition property', 'hello2-forms' );
		...
		if ( ! isset( $condition['value']['property_meta']['type'] ) ) {
			$this->isConditionCorrect = false;
			$this->conditionErrors[]  = esc_html__( 'missing condition property type', 'hello2-forms' );
		...
		if ( ! isset( $condition['value']['operator'] ) ) {
			$this->isConditionCorrect = false;
			$this->conditionErrors[]  = esc_html__( 'missing condition operator', 'hello2-forms' );
		...
		if ( ! isset( $condition['value']['value'] ) ) {
			$this->isConditionCorrect = false;
			$this->conditionErrors[]  = esc_html__( 'missing condition value', 'hello2-forms' );
		...
		if ( ! isset( self::CONDITION_MAPPING[ $typeField ] ) ) {
			$this->isConditionCorrect = false;
			$this->conditionErrors[]  = esc_html__( 'configuration not found for condition type', 'hello2-forms');
			return;
		}

		if ( ! isset( self::CONDITION_MAPPING[ $typeField ]['comparators'][ $operator ] ) ) {
			$this->isConditionCorrect = false;
			$this->conditionErrors[]  = esc_html__( 'configuration not found for condition operator', 'hello2-forms' );
		...
		if ( is_array( $type ) ) {
			...
			if ( ! $this->valueHasCorrectType( $type, $value ) ) {
				$this->isConditionCorrect = false;
				$this->conditionErrors[]  = esc_html__( 'wrong type of condition value', 'hello2-forms' );
			}
		}
	}
	...
	private function checkConditions( $conditions ) {
		if ( array_key_exists( 'operatorIdentifier', $conditions ) ) {
			if ( ( $conditions['operatorIdentifier'] !== 'and' ) && ( $conditions['operatorIdentifier'] !== 'or' ) ) {
				$this->conditionErrors[]  = esc_html__( 'missing operator', 'hello2-forms' );
				$this->isConditionCorrect = false;
			...
			if ( isset( $conditions['operatorIdentifier']['children'] ) ) {
				$this->conditionErrors[]  = esc_html__( 'extra condition', 'hello2-forms' );
				$this->isConditionCorrect = false;
			...
			if ( ! is_array( $conditions['children'] ) ) {
				$this->conditionErrors[]  = esc_html__( 'wrong sub-condition type', 'hello2-forms' );
				$this->isConditionCorrect = false;
		...
		if ( is_array( $conditions ) && count( $conditions ) > 0 ) {
			foreach ( $conditions as $val ) {
				if ( ! in_array( $val, static::ACTIONS_VALUES ) ||
				     ( in_array( $this->field["type"], [
						     'nf-text',
						     'nf-code',
						     'nf-page-break',
						     'nf-divider',
						     'nf-image'
					     ] ) && ! in_array( $val, [ 'hide-block' ] ) ) ||
				     ( isset( $this->field["hidden"] ) && $this->field["hidden"] && ! in_array( $val, [
						     'show-block',
						     'require-answer'
					     ] ) ) ||
				     ( isset( $this->field["required"] ) && $this->field["required"] && ! in_array( $val, [
						     'make-it-optional',
						     'hide-block',
						     'disable-block'
					     ] ) ) ||
				     ( isset( $this->field["disabled"] ) && $this->field["disabled"] && ! in_array( $val, [
						     'enable-block',
						     'require-answer',
						     'make-it-optional'
					     ] ) )
	...
	public function message() {
		...
		if ( ! $this->isConditionCorrect ) {
			$message = esc_html__('The logic conditions for ', 'hello2-forms') . $this->field['name'] . esc_html__( ' are not complete.', 'hello2-forms' );
		} else if ( ! $this->isActionCorrect ) {
			$message = esc_html__('The logic actions for ', 'hello2-forms') . $this->field['name'] . esc_html__(' are not valid.', 'hello2-forms');
		}
		if ( count( $this->conditionErrors ) > 0 ) {
			return $message . esc_html__(' Error detail(s): ', 'hello2-forms') . implode( ', ', $this->conditionErrors );
		}
