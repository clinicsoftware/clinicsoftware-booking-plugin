...
use Hello2Forms\Architecture\Form;
use Hello2Forms\Architecture\ValidatorRule;
use Hello2Forms\Rules\FormPropertyLogicRule;
use Hello2Forms\Rules\OneEmailPerLine;
use Illuminate\Validation\Rule;

class UserFormValidator extends ValidatorRule {
	/**
	...
	public function rules(): array {
		return [
			...
			'properties.*.width'                       => [
				'sometimes',
				Rule::in( [ 'full', '1/2', '1/3', '2/3', '1/3', '3/4', '1/4' ] )
			],
			'properties.*.align'                       => [
				'sometimes',
				Rule::in( [ 'left', 'center', 'right', 'justify' ] )
			],
	...
	public function messages(): array {
		return [
			'properties.*.name.required'      => esc_html__('The form block number :position is missing a name.', 'hello2-forms'),
			'properties.*.type.required'      =>  esc_html__('The form block number :position is missing a type.', 'hello2-forms'),
			'properties.*.max_char_limit.min' =>  esc_html__('The form block number :position max character limit must be at least 1 OR Empty', 'hello2-forms'),
			'properties.*.max_char_limit.max' =>  esc_html__('The form block number :position max character limit may not be greater than 2000.', 'hello2-forms'),
		];
	}
}
