...
/**
 * FROM https://github.com/ClinicSoftware-com/ClinicSoftware-PHP-API/blob/master/examples/add_lead.php
 * pls no hard reject for this :(
 */
class Salon_api
{
	...
	public function call($params = array())
	{
		...
		$params['api_client_salt'] = uniqid( (string) wp_rand(), true );
		...
		if ($this->debug) {
			$log_buffer .= "Call to {$this->apiURL}: " . json_encode($params) . "\n\n";
		}
		...
		if (is_wp_error($response)) {
			$this->last_error = "API call to {$this->apiURL} failed: " . $response->get_error_message();

			if ($this->debug) {
				$log_buffer .= $this->last_error . "\n\n";
				$log_buffer .= 'Completed in ' . number_format($time * 1000, 2) . " ms\n\n";
				$this->writeLog($log_buffer);
			}
		...
		if ($this->debug) {
			$log_buffer .= 'Completed in ' . number_format($time * 1000, 2) . " ms\n\n";
			$log_buffer .= 'Got response (' . $http_code . '): ' . $response_body . "\n\n";
			$this->writeLog($log_buffer);
		}
	...
	public function readLog()
	{
		...
		if (!$wp_filesystem->exists($this->log_filename)) {
			return '';
		}

		return $wp_filesystem->get_contents($this->log_filename);
	}
	...
	private function writeLog($message)
	{
		...
		$existing = $wp_filesystem->get_contents($this->log_filename);
		if (false === $existing) {
			$existing = '';
		}
	...
	private function clearLog()
	{
		...
		$wp_filesystem->put_contents($this->log_filename, '', FS_CHMOD_FILE);
	}
}
	...
	public function toApiJSON(): array {
		...
		{ // Error checking
			...
			if ( empty($this->datetime) || $this->datetime->getTimestamp() < time() )
				throw new Exception( esc_html( 'Invalid DateTime: ' . $this->datetime->getTimestamp() . ' < ' . time() ) );
