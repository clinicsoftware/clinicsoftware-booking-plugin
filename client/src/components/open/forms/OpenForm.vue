<template>
  ...
  <form v-else-if="dataForm" @submit.prevent="">
    ...
    <!-- Captcha -->
    <template v-if="form.use_captcha && isLastPage && !adminPreview">
...
export default {
  ...
  data() {
    return {
       ...
       */
      formVersionId: 1,
      darkModeEnabled: (document.querySelector('.hello2-forms-app') || document.body).classList.contains('dark'),
      isAutoSubmit: false,
      /**
  ...
  computed: {
    hCaptchaSiteKey: () => window.hello2formsConfig.hCaptchaSiteKey,
    /**
  ...
  watch: {
    ...
    dataForm: {
      deep: true,
      handler() {
        if (this.isPublicFormPage && this.form && !Object.values(this.dataFormValue).every(x => x === null || x === '')) {
          try {
  ...
  methods: {
    ...
    async getSubmissionData() {
      ...
      await this.$store.dispatch('open/records/loadRecord',
        axios.get('/wp-json/hello2forms/v1/FormController/' + this.form.slug + '/submissions/' + this.form.submission_id).then((response) => {
    ...
    async initForm() {
      ...
      if (this.isPublicFormPage && this.form.editable_submissions) {
        if (this.$route.query.submission_id) {
          this.form.submission_id = this.$route.query.submission_id
      ...
      if (this.isPublicFormPage) {
        urlPrefill = new URLSearchParams(window.location.href.split('?')[1])
      }
