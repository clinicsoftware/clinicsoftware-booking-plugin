<template>
  <div v-if="form" class="open-complete-form">
    ...
    <div v-if="isPublicFormPage && form.is_password_protected && !passwordCorrect">
    ...
    <v-transition>
      <div v-if="form.is_password_protected && !hidePasswordDisabledMsg"
    ...
    <transition
      v-if="(!form.is_password_protected || passwordCorrect) && (!isPublicFormPage || (!form.is_closed && !form.max_number_of_submissions_reached && form.visibility!='closed'))"
    ...
    >
      <div v-if="!submitted" key="form">
        ...
        <p v-if="!form.no_branding" class="text-center w-full mt-2">
          <a href="https://clinicsoftware.com?utm_source=form&utm_content=powered_by"
             ...
             target="_blank"
          >Powered by <span class="font-semibold">{{ appName }}</span></a>
        </p>
      </div>
      <div v-else key="submitted" class="px-2">
        ...
        <p v-if="!form.no_branding" class="mt-5">
          <a target="_parent" href="https://clinicsoftware.com/?utm_source=form&utm_content=create_form_free" class="text-nt-blue hover:underline">
            Powered By {{ appName }}
          </a>
        </p>
      </div>
    ...
    <stripe-checkout
      ...
      ref="checkoutRef"
      mode="payment"
      :pk="stripeData.data.account.data.pk"
      :line-items="parseLineItems()"
      :success-url="stripeData.data.successURL"
      :cancel-url="stripeData.data.cancelURL"/>
    <GoogleCalendarPopup v-if="hasGooglePopup && submitted" :type="form.submission_mode" :extraData="this.form.submission_extra_data" :eventName="form.title"/>
  </div>
</template>
...
import GoogleCalendarPopup from "../../common/GoogleCalendarPopup.vue";
...
export default {
  components: {GoogleCalendarPopup, VTransition, VButton, OpenFormButton, OpenForm, FormCleanings, StripeCheckout },
  ...
  computed: {
    appName () {
      return window.hello2formsConfig.app_name;
    },
    isIframe () {
    ...
    hasGooglePopup () {
      return this.form.submission_mode === 'add_to_google_calendar' ||  this.form.submission_mode === 'add_to_google_meet';
    }
  },

  mounted () {
    this.stripeData = this.hasStripeAction(this.form.actions)
  },

  methods: {
    hasStripeAction(actions) {
      for (let action of actions) {
        if (action.type === 'payment.stripe') {
          return action;
        }
      }
    },
    parseLineItems() {
      return this.stripeData.data.lineItems.map(item => {
        return {
          price: item,
          quantity: 1
        }
    ...
    passwordEntered () {
      axios.post('/wp-json/hello2forms/v1/FormController/' + this.form.slug  + '/password', {password: this.passwordForm.password}).then((response) => {
        this.passwordCorrect = true

        loadForm(this.form.slug)
      }).catch(() => {
    ...
    submitForm (form, onFailure) {
      ...
      this.loading = true
      this.closeAlert()
      form.post('/wp-json/hello2forms/v1/FormController/' + this.form.slug + '/answer').then((response) => {
