<template>
  <div v-if="field" class="py-2">
    ...
    <div v-if="['select','multi_select'].includes(field.type)" class="border-b py-2 px-4">
      ...
      <text-area-input ref="selct" :value="optionsText" :name="field.id+'_options_text'" class="mt-3"
    ...
    <div v-if="displayBasedOnAdvanced" class="border-b py-2 px-4">
      ...
      <template v-if="hasCrmAction">
        <text-input name="crm_map" class="mt-3"
                    :form="field" :required="false"
                    label="CRM Map Name"
        />

        <select-input name="crm_custom_field" class="mt-3"
                      :form="field" :options="crmMappings.map((ac) => {
                                              return {
                                                value: ac.id,
                                                name: ac.field_label,
                                              }
                                            })"
                      label="CRM Custom Field"
        />
      </template>
...
export default {
  ...
  computed: {
    hasCrmAction() {
      for (let action of this.form.actions) {
        if (action.type === 'webhook.clinic_software.cms') {
          return action
        }
      }
    },
  ...
  watch: {
    ...
    'field.crm_custom_field': {
      handler(val) {
        if (!val) return;

        const mappings = this.crmMappings.find((ac) => {
          return ac.id === val
        })

        if (mappings.field_values && mappings.field_type === 'list') {
          console.log(this.$refs.selct)
          this.$refs.selct._data.content = mappings.field_values.split(',').join('\n')
          this.$set(this.field, 'select',
            {
              "options": mappings.field_values.split(',').map(name => {
                return {
                  name: name,
                  id: name
                }
    ...
    'form.actions': {
      handler(val) {
        if (this.hasCrmAction) {
          axios.get('/wp-json/hello2forms/v1/IntegrationsController/crm/custom-fields/' + this.hasCrmAction.data.account)
            .then(response => {
              this.crmMappings = response.data.data
            })
        }
      },
      immediate: true
