<template>
  <div v-if="showSidebar"
    ...
    <template v-if="field">
      <webhook-action :form="form" v-if="field.type ==='webhook'" :field="field"/>
    </template>
...
import WebhookAction from "./components/actions/WebhookAction.vue";

export default {
  name: 'FormActionEditSidebar',
  components: {ChangeFieldType, FieldOptions, BlockOptions, WebhookAction},
  props: {},
  data() {
    return {
  ...
  computed: {
    ...mapState({
      ...
      showEditFieldSidebar: state => state['open/action_form'].showEditFieldSidebar
    }),
    form: {
      get() {
    ...
    field() {
      return (this.form && this.selectedFieldIndex !== null) ? this.form.actions[this.selectedFieldIndex] : null
    },
    showSidebar() {
      return (this.form && this.selectedFieldIndex !== null) ? (this.form.actions[this.selectedFieldIndex] && this.showEditFieldSidebar) : false
    },
    isBlockField() {
  ...
  methods: {
    ...
    duplicateBlock() {
      const newFields = clonedeep(this.form.actions)
      const newField = clonedeep(this.form.actions[this.selectedFieldIndex])
    ...
    closeSidebar() {
      this.$store.commit('open/action_form/closeEditFieldSidebar')
    },
    generateUUID() {
