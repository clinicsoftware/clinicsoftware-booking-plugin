  ...
  <collapse class="p-4 w-full border-b" :default-value="isCollapseOpen" @click="onClickCollapse">
    ...
    <select-input :form="submissionOptions" name="submissionMode" label="Post Submission Action"
                  :options="[
                    ...
                    {name:'Add to Google Calendar', value:'add_to_google_calendar'},
                    {name:'Add to Google Meet', value:'add_to_google_meet'}
    ...
    <template
      v-if="submissionOptions.submissionMode === 'add_to_google_calendar' || submissionOptions.submissionMode === 'add_to_google_meet'">
      <toggle-switch-input name="submission_extra_data_allowUserToSelect"
                           @input="saveToSubmissionExtraData('allowUserToSelect', $event)"
                           class="mt-4">
        <template #label>
          Allow User to select dates
        </template>
      </toggle-switch-input>
      <template v-if="(form.submission_extra_data.allowUserToSelect === undefined || !form.submission_extra_data.allowUserToSelect )
&& (submissionOptions.submissionMode === 'add_to_google_calendar' || submissionOptions.submissionMode === 'add_to_google_meet')">
        <date-input :with-time="true" name="submission_extra_data_start_date"
                    @input="saveToSubmissionExtraData('start_date', $event)"
                    label="Start Date"
                    :required="!form.submission_extra_data.allowUserToSelect"
        />
        <date-input :with-time="true" name="submission_extra_data_end_date"
                    v-model="form.submission_extra_data.end_date"
                    @input="saveToSubmissionExtraData('end_date', $event)"
                    label="End Date"
                    :required="!form.submission_extra_data.allowUserToSelect"
        />
      </template>
...
export default {
  ...
  watch: {
      ...
      handler() {
        if (this.form) {
          this.submissionOptions = {
            submissionMode: this.form.redirect_url ? 'redirect' : this.form.submission_mode,
    ...
    submissionOptions: {
      ...
      handler: function (val) {
        if (val.submissionMode !== this.form.submission_mode) {
          this.$set(this.form, 'submission_mode', val.submissionMode)
        }
  ...
  mounted() {
    this.isMounted = true
    if (this.isObjectEmpty(this.form.submission_extra_data)) {
      this.$set(this.form, 'submission_extra_data', {
        start_date: new Date(),
        end_date: new Date('+1 hour'),
        allowUserToSelect: false
      })
    }
  },

  methods: {
    saveToSubmissionExtraData(type, e) {
      const objCopy = {...this.form.submission_extra_data ?? {}};
      objCopy[type] = e ;
      this.$set(this.form, 'submission_extra_data', objCopy);
    },
    onClickCollapse(e) {
