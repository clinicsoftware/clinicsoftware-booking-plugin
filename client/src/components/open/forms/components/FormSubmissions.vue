...
export default {
  ...
  computed: {
    ...
    exportUrl() {
      ...
      return '/wp-json/hello2forms/v1/FormController/' + this.form.id + '/submissions/export'
    },
    filteredData () {
  ...
  methods: {
    ...
    getSubmissionsData() {
      ...
      this.isLoading = true
      axios.get('/wp-json/hello2forms/v1/FormController/' + this.form.slug + '/submissions?page=' + this.currentPage).then((response) => {
        const resData = response.data

        this.tableData = this.tableData.concat(resData.data.map((record) =>
          ({
            ...record.data,
            id: record.id
          })))
